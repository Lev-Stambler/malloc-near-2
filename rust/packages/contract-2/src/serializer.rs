...
impl Contract {
    pub(crate) fn deserialize_node(serialized: SerializedNode, prefix_base: &str) -> Node {
        ...
        match serialized {
            SerializedNode::MallocCall {
                ...
                gas,
                next_splitters: Contract::vec_to_vector(&next_splitters, node_prefix.as_bytes()),
            },
        }
    }
    ...
    pub(crate) fn deserialize(&self, splitter: &SerializedSplitter, ephemeral: bool) -> Splitter {
        ...
        Splitter {
            ...
            owner,
            ft_contract_id: splitter.ft_contract_id.clone(),
        }
    }

    pub(crate) fn vec_to_vector<T: BorshSerialize>(v: &[T], prefix: &[u8]) -> Vector<T> {
